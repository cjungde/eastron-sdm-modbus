...
from eastron_sdm_modbus import SDM72D, SDM72DMeter
...
async def test_every_field_decodes(meter: SDM72DMeter) -> None:
    ...
    assert m.voltage_l1 == pytest.approx(226.5671)
    assert m.voltage_l2 == pytest.approx(226.3041)
    assert m.voltage_l3 == pytest.approx(226.1867)
    assert m.current_l1 == pytest.approx(0.296)
    assert m.current_l2 == pytest.approx(0.0)
    assert m.current_l3 == pytest.approx(0.2481)
    assert m.power_l1 == pytest.approx(9.8422)
    assert m.power_l2 == pytest.approx(0.0)
    assert m.power_l3 == pytest.approx(3.5717)
    assert m.avg_voltage == pytest.approx(226.3119)
    assert m.total_power == pytest.approx(14.245)
    assert m.total_va == pytest.approx(123.4009)
    assert m.total_var == pytest.approx(-116.3244)
    assert m.power_factor == pytest.approx(0.1154)
    assert m.frequency == pytest.approx(50.09)
    assert m.import_energy == pytest.approx(17613.9707)
    assert m.export_energy == pytest.approx(0.0)
    assert m.neutral_current == pytest.approx(0.2758)
    assert m.total_energy == pytest.approx(17613.9707)
    assert m.resettable_import == pytest.approx(330.962)
    assert m.resettable_export == pytest.approx(0.0)
    assert m.net_energy == pytest.approx(17613.9707)
